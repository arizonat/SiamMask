...
class Custom(SiamMask):
    ...
    def comp_latent(self, t1, t2):
        rpn_pred_cls, rpn_pred_loc = self.rpn(t1, t2)
        return rpn_pred_cls, rpn_pred_loc

    def track_mask_latent(self, zf, search):
        feature, search = self.features.forward_all(search)
        rpn_pred_cls, rpn_pred_loc = self.rpn(zf, search)
        corr_feature = self.mask_model.mask.forward_corr(zf, search)
        pred_mask = self.mask_model.mask.head(corr_feature)
        return rpn_pred_cls, rpn_pred_loc, pred_mask, feature, corr_feature

    def track_refine_latent(self, pos, feature, corr_feature):
        pred_mask = self.refine_model(feature, corr_feature, pos=pos, test=True)
        return pred_mask
